...
test("writeDaemonInfoAt then readDaemonInfoAt round-trips workspace, session and port", () => {
...
test("writeDaemonInfoAt records a non-default session name too", () => {
  let p = freshInfoPath("roundtrip-named");
  writeDaemonInfoAt(p, "/some/workspace", "review", 8342);
  let info = readDaemonInfoAt(p);
  expect(info != null);
  if (info != null) { expect(info.session == "review"); }
});
...
test("two named sessions on the same workspace usually land on different ports too", () => {
  let a = portFromWorkspace("/tmp/workspace-a", "review", 9000, 500);
  let b = portFromWorkspace("/tmp/workspace-a", "release", 9000, 500);
  let unnamed = portFromWorkspace("/tmp/workspace-a", "", 9000, 500);
  expect(a != b);
  expect(a != unnamed);
  expect(b != unnamed);
});
...
test("the POSIX spawn command backgrounds the daemon and does not block the caller", () => {
  ...
  expect(cmd.indexOf("JOULE_SESSION_NAME") < 0);
  expect(cmd.indexOf("/opt/joule-code/bin/joule-daemon") >= 0);
});

test("the POSIX spawn command carries a session name through as an env var when asked, and nothing when not", () => {
  let named = posixDaemonSpawnCommand("/tmp/some-workspace", "review", 9100, "/tmp/some.log", false, "/opt/joule-code/bin/joule-daemon");
  expect(named.indexOf("JOULE_SESSION_NAME='review'") >= 0);
  let unnamed = posixDaemonSpawnCommand("/tmp/some-workspace", "", 9100, "/tmp/some.log", false, "/opt/joule-code/bin/joule-daemon");
  expect(unnamed.indexOf("JOULE_SESSION_NAME") < 0);
});
...
test("the Windows spawn command carries a session name through as an env var when asked, and an empty one when not", () => {
  let named = windowsDaemonSpawnCommand("C:\\ws", "review", 9100, "C:\\logs\\d.log", false, "C:\\bin\\joule-daemon.exe");
  expect(named.indexOf("$env:JOULE_SESSION_NAME = 'review'") >= 0);
  let unnamed = windowsDaemonSpawnCommand("C:\\ws", "", 9100, "C:\\logs\\d.log", false, "C:\\bin\\joule-daemon.exe");
  expect(unnamed.indexOf("$env:JOULE_SESSION_NAME = ''") >= 0);
});
