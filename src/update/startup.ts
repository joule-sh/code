...
import { resolveInstallRoot, detectRunningExePath, detectInstallMethod, INSTALL_ROOT_ENV } from "./install_detect.ts";
...
export function maybeStartUpdateCheck(nonce: string): UpdateNotifier {
  ...
  let home = homeDir();
  let envDisable = envOr(UPDATE_CHECK_ENV, "");
  ...
  if (shouldCheck(VERSION, disabled, method, cache, Date.now())) {
    notifier.start(nonce, VERSION, method, cachePath);
  }
  return notifier;
}
