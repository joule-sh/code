...
import { InstallResult, runInstallOnce, RESULT_INSTALLED, RESULT_UP_TO_DATE } from "./installer.ts";
import { runNpmUpdate } from "./npm_update.ts";
import { INSTALL_METHOD_NPM } from "./install_detect.ts";
...
let g_method: string = "";
let g_exe_path: string = "";
let g_install_root: string = "";
let g_bin_dir: string = "";
let g_mailbox: string = "";

export function configureInstallWorker(currentVersion: string, method: string, exePath: string, installRoot: string, binDir: string, mailboxPath: string): void {
...
function runConfiguredUpdate(): InstallResult {
  if (g_method == INSTALL_METHOD_NPM) {
    return runNpmUpdate(g_current_version, g_exe_path);
  }
  return runInstallOnce(g_current_version, g_install_root, g_bin_dir);
}

export function installWorkerLoop(): int {
  let result = runConfiguredUpdate();
