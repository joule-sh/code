...
**HTTP + SSE, not WebSocket.** Lumen has no WebSocket: `lumen_runtime_net.zig`
answers an upgrade with `426 Upgrade Required`, and the stdlib check file says so
in as many words. What it *does* have is `http.stream` (spec 452) — a live read
handle over a response in progress, chunked-decoding, `Accept-Encoding: identity`,
written for token streams — plus chunked response writing on the server side. So
every channel in the diagram is an SSE downstream with POSTs going the other way.
This is not a workaround we tolerate; it traverses proxies better than WebSocket
and it is the transport the toolchain is actually good at.
